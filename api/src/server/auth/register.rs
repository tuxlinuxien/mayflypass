    ...
    fn validate(&mut self) -> Result<(), ApiError> {
        ...
        let errors: Vec<FieldError> = vec![
            ...
            FieldError::check_required("username", &self.username)
                .or(FieldError::check_username_invalid(
                    "username",
                    &self.username,
                ))
    ...
    match database::account::insert(&state.pool, new_user).await {
        ...
        Err(e) => match e {
            database::error::Error::UniqueViolation { .. } => {
                return Err(ApiError::BadRequestFieldErrors(vec![
                    FieldError::ValueDuplicated("username".into()),
                ]));
            }
            _ => {
...
mod test {
    ...
    async fn test_register_validate() {
        ...
        assert_eq!(val, vec![FieldError::UsernameInvalid("username".into())]);
        ...
        assert_eq!(val, vec![FieldError::ValueRequired("username".into())]);
        ...
        // username too short
        let mut input = base.clone();
        input.username = "mini".into();
        let res = input.validate();
        assert!(res.is_err());
        let ApiError::BadRequestFieldErrors(val) = res.err().unwrap() else {
            panic!("BadRequestFieldErrors");
        };
        assert_eq!(val, vec![FieldError::ValueTooShort("username".into(), 5)]);
